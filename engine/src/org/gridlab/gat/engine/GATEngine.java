...
public class GATEngine {
    ...
    protected void loadAPKs() {
        ...
        if (adaptorPath != null) {
            ...
            File[] adaptorAPKs = new File[] {
                    new File(adaptorPath + File.separator
                            + "org.gridlab.gat.android.adaptor.Local.apk"),
                    // new File(adaptorPath + File.separator
                    // + "org.gridlab.gat.android.adaptor.Zorilla.apk"),
                    new File(adaptorPath + File.separator
                            + "org.gridlab.gat.android.adaptor.Ssh.apk") };
            // File[] adaptorAPKs = adaptorRoot.listFiles(new FileFilter() {
            // public boolean accept(File file) {
            // System.out.println("file found: " + file.getName());
            // return file.isFile() && file.getName().endsWith(".apk");
            // }
            // });
            if (adaptorAPKs.length == 0) {
                throw new Error("gat.adaptor.path set to '" + adaptorPath
                        + "', but it doesn't contain any adaptor");
            }
            HashMap<String, ClassLoader> adaptorClassLoaders = new HashMap<String, ClassLoader>();
            for (File adaptorAPK : adaptorAPKs) {
                System.err.println("loading... " + adaptorAPK.getName());
                try {
                    adaptorClassLoaders.put(adaptorAPK.getName(),
                            loadAPK(adaptorAPK));
                } catch (Exception e) {
                    if (logger.isDebugEnabled()) {
                        logger.debug("Unable to load adaptor '"
                                + adaptorAPK.getName() + "': " + e);
                        e.printStackTrace();
                    }
                }
            }
    ...
    private ClassLoader loadAPK(File apkFile) throws Exception {
        JarFile adaptorJar = new JarFile(apkFile, true);
        JarEntry jarEntry = adaptorJar.getJarEntry("assets/ADAPTORMANIFEST.MF");
        Manifest m = new Manifest(adaptorJar.getInputStream(jarEntry));
        Attributes attributes = m.getMainAttributes();
        Set<Object> keys = attributes.keySet();
        ClassLoader adaptorLoader = new ApkClassLoader(apkFile, this.getClass()
                .getClassLoader());
        for (Object key : keys) {
    ...
    public static String defaultMarshal(Object o) {
        ...
        throw new Error("could not marshal object: ");
    }
    ...
    public static Advertisable defaultUnmarshal(Class<?> type, String s) {
        ...
        throw new Error("could not unmarshal object: ");
    }
