...
public class ServerListItem extends JPanel implements ActionListener {
	 ...
	 * @param buttonText
	 */
	public static JButton createImageButton(ImageIcon icon, String buttonText) {
	...
	ServerListItem(ServerHandler handler) {
		...
		button = createImageButton(playIcon, null);
	...
	public void updateLabelColor() {
		// update label color to black if server is now active
		if (!handler.isActive()) {
			label.setForeground(Color.LIGHT_GRAY);
		} else {
			if (handler.isEnabled()) {
