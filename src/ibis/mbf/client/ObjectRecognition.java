...
public class ObjectRecognition
{
    ...
    private double weibullDiff(double gam1, double bet1,
            double gam2, double bet2)
	{
...
//      get score for 1 receptive field over all invariants
        ...
        for(int i=0; i<NR_INVARS; i++) {
            score += weibullDiff(data1[i*2], data1[i*2+1],
...
//      TODO: implement
    ...
    public synchronized String recognize(FeatureVector vector)
	{
        ...
        try {
//          EEP: we open a file on every recognize! FIXME!
            ...
            while ((objNames[j] = buf.readLine()) != null) {
                ...
                for (int i=0; i<oldWeibs.length; i++) {
                    oldWeibs[i] =
						(Double.valueOf(buf.readLine()).doubleValue());
                }
    ...
    public synchronized
	boolean learn(String name, FeatureVector vector)
	{ 
        double [] weibulls = vector.vector;
        ...
        try {
            FileOutputStream outFile =
							new FileOutputStream(OBJFILE, true);
            OutputStreamWriter out =
							new OutputStreamWriter(outFile, "US-ASCII");
            out.write(name + "\n");
