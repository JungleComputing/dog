...
public class CxWeibull {
    private static String INFILE = "/home/fjseins/Java/SCALE/Images/hedgehog.jpg";

    private static String OUTFILE = "/home/fjseins/Java/SCALE/Images/outfile.jpg";
    ...
    // private static int NR_RFIELDS = 20;
    ...
    private static void initialize(int width, int height) {
        ...
        double sigma = 30.0 * (double) height / 576;
        int centerx = width / 2;
        int centery = height / 2;

        CxArray2dScalarDouble pntIm = new CxArray2dScalarDouble(width, height);
        ...
        for (int n = 1; n <= order; n++) {
            ...
            for (int phi = 0; phi < n * radialdens; phi++) {
                ...
                // if (x >= sigma && y >= sigma && (x < width - sigma)
                // && (y < height - sigma)) {
                pntIm.setSingleValue(one, x, y, true);
                rfIm[idx] = pntIm.gaussDerivative(sigma, 0, 0, 5.);
                if (rfIm[idx] == null) {
                    System.out.println("rfIm[idx]==null for sigma: " + sigma);
                }
        ...
        System.out.println("Number of rfIms: " + idx);
    ...
    public static void doRecognize(int width, int height, byte[] bArray,
        ...
        for (int i = 0; i < NR_RFIELDS; i++) {
            if (rfIm[i] == null) {
                System.out.println("rfIm[i] == null, for i: " + i);
            }
