...
public class CxWeibull {
    ...
    private static int NR_INVAR_IMS = 5;

    private static int NR_INVARS = (NR_INVAR_IMS - 1) / 2;

    private static int NR_RFIELDS = 7;

    private static int NR_BINS = 500;
    ...
    private static double[][] resbetas = new double[NR_INVARS][NR_RFIELDS];

    private static double[][] resgammas = new double[NR_INVARS][NR_RFIELDS];
    ...
    public static void initialize() {
        try {
            File file = new File("/data/local/rfIms.init");
            ObjectInputStream in = new ObjectInputStream(new FileInputStream(
                    file));
            rfIm = (CxArray2dScalarDouble[]) in.readObject();
            System.out.println("Done reading rfims to disk!");
        } catch (Exception e) {
    ...
    public static void initialize(int inImW, int inImH) {
        ...
        CxArray2dScalarDouble pntIm = new CxArray2dScalarDouble(inImW, inImH);
        ...
        for (int n = 1; n <= order; n++) {
            ...
            for (int phi = 0; phi < n * radialdens; phi++) {
                ...
                rfIm[idx] = pntIm.testgaussDerivative(sigma, 0, 0, 5.);
    ...
    public static void doRecognize(int width, int height, byte[] bArray,
        ...
        System.out.println("betas: " + Arrays.deepToString((betas)));
        System.out.println("gammas: " + Arrays.deepToString((gammas)));
        ...
        for (int j = 1; j < NR_INVARS * 2 + 1; j += 4) {
            for (int i = 0; i < NR_RFIELDS; i++) {
                xxb = 0.;
                xyb = 0.;
                yyb = 0.;
     ...
     * System.exit(0); } // Reading image data from file //
     ...
     * initialize(width, height); // System.out.println("Done."); } // Create
     * CxArray2d from file image data
     * 
     * CxArray2dVec3Double input = new CxArray2dVec3Double(width, height,
     * CxConvert.toDoubles(bArray)); // Create all invariant images //
     * System.out.println("Building invariant images...");
     * buildInvariantImages(input); // System.out.println("Done."); //
     * Initialize histos, betas, and gammas // System.out.println("Initializing
     * histos, betas, and gammas..."); for (int j=0; j<NR_INVAR_IMS; j++) { for
     * (int i=0; i<NR_RFIELDS; i++) { betas[j][i] = 0.; gammas[j][i] = 0.; for
     * (int k=0; k<NR_BINS; k++) { histos[j][i][k] = 0.; } } } //
     * System.out.println("Done."); // Calculate all histograms //
     * System.out.println("Calculating all histograms..."); for (int i=0; i<NR_RFIELDS;
     * i++) {
     * 
     ...
     * System.out.println("Done."); // Calculate all Weibull fits //
     * System.out.println("Calculating all Weibull fits..."); CxWeibullFit fit =
     * new CxWeibullFit(); fit.init(new FitWeibull(), histos, betas, gammas,
     * NR_BINS); CxPatTask.dispatch(fit, NR_RFIELDS, NR_INVAR_IMS); //
     * System.out.println("Done."); // Extract all final betas and gammas
     * (ignore EX2EY2)
     * 
     ...
     * resgammas[((j-1)/2)+1][i] = 2.0*Ib/4.; } } // Make invalid all parameters
     * that are not finite or too large
     * 
     ...
     * resgammas[j][i] > 100.) { invalids[i] = true; } } } // Finalize the set
     * of Weibull parameters and... done!
     * 
