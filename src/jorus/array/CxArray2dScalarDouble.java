...
import jorus.operations.CxBpoToHistDouble;
import jorus.operations.CxGenConv2dSepGauss;
import jorus.operations.CxSetBorderMirrorDouble;
import jorus.patterns.CxPatBpoToHist;
import jorus.patterns.CxPatGenConv2dSep;
...
public class CxArray2dScalarDouble extends CxArray2dDoubles {
     ...
     * 
     */
    private static final long serialVersionUID = -5343131942171568361L;
    ...
    public CxArray2dScalarDouble testgaussDerivative(final double sigma,
            final int orderX, final int orderY, final double truncation) {
        CxArray2dScalarDouble gx = CxGaussian1d.create(sigma, orderX,
                3 * sigma * 2 + 1, width / 2, 0);
        CxArray2dScalarDouble gy = CxGaussian1d.create(sigma, orderY,
                3 * sigma * 2 + 1, height / 2, 0);
        return (CxArray2dScalarDouble) CxPatGenConv2dSep.dispatch(this, gx, gy,
                new CxGenConv2dSepGauss(), new CxSetBorderMirrorDouble());
    }

    public CxArray2dScalarDouble gaussDerivative(final double sigma,
            final int orderX, final int orderY, final double truncation) {
        return gaussDerivative(sigma, sigma, orderX, orderY, truncation);
    }

    public CxArray2dScalarDouble gaussDerivative(final double sx,
            final double sy, final int orderX, final int orderY,
