...
public abstract class CxArray2dDoubles extends CxArray2d<double[]> {
    ...
    public CxArray2d setSingleValue(CxPixel p, int xidx, int yidx, boolean inpl) {
        if (!equalExtent(p))
            return null;
        return CxPatSvo.dispatch(this, xidx, yidx, inpl, new CxSvoSetDouble(
                (double[]) p.getValue()));
    }
    ...
    public CxArray2d setVal(CxPixel p, boolean inpl) {
        if (!equalExtent(p))
            return null;
        return CxPatUpo.dispatch(this, inpl, new CxUpoSetValDouble((double[]) p
                .getValue()));
    }
    ...
    public CxArray2d mulVal(CxPixel p, boolean inpl) {
        if (!equalExtent(p))
            return null;
        return CxPatUpo.dispatch(this, inpl, new CxUpoMulValDouble((double[]) p
                .getValue()));
    }
